...
const getUserPlaylists = asyncHandler(async (req, res) => {
    ...
    const playlists = await Playlist.find({
        owner: userId
    }).populate("owner", "username fullName avatar").sort({ createdAt: -1 }).skip(skip).limit(limitNum);
...
const getPlaylistById = asyncHandler(async (req, res) => {
    ...
    const skip = (pageNum - 1) * limitNum;
    const playlist = await Playlist.findById(playlistId).populate("owner", "username fullName avatar").lean();
    if (!playlist) {
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if (playlist.videos.includes(videoId)) {
        ...
        await playlist.save();
        return res.status(200).json(new ApiResponse(playlist, 200, "Video added to playlist successfully"));
    }
});
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(playlist, 200, "Video removed from playlist successfully"));
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    const updatedPlaylist = await Playlist.findByIdAndUpdate(playlistId, updateData, { new: true }).populate("owner", "username fullName avatar");
    return res.status(200).json(new ApiResponse(updatedPlaylist, 200, "Playlist updated successfully"));
});
